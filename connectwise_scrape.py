...
def header_build(_cw_company, _cw_api_id, _cw_api_key, _cw_agentId, addnlheaderitems = {}):
	"""
	Builds the headers for use with the ConnectWise APIs which can be included
	in the requests `headers=` parameter.
	...
	Params:
		...
		_cw_api_key
		_cw_agentId
		addnlheaderitems dict : A dictionary of additionl headers and values

	Returns: A dictionary of the headers to include.
	"""
	header_dict = {}
...
if __name__ == '__main__':
	...
	if raw_response['code'] in (200, 201):
		...
		for item in devices:
			...
			#model
			# FIX: * Doesn't check values found.  Last found is set regardless of value
			...
			for k,v in all_properties.items():
				if 'serial' in k:
					# FIX:  * This could potentially overwrite serialNumber multiple times
